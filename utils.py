...
def limpiar_caracteres_repetidos(texto):
    """Antiflood: Limita caracteres repetidos"""
    resultado = ""
    char_anterior = ""
    contador = 0
...
def procesar_comando(comando, clientes, historial):
    """Procesa comandos especiales y retorna la respuesta"""
    ...
    if comando == "/hist":
        ...
        for i, info in enumerate(clientes.values(), 1):
            nombre = info['nombre']
            ip = info['ip'] 
            puerto = info.get('puerto', 'N/A')  # Por compatibilidad con versiones anteriores
