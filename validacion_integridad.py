...
class ValidadorIntegridad:
    ...
    def agregar_hash(mensaje, mensaje_cifrado):
        """
        Agrega el hash SHA-256 al mensaje cifrado
        
        Args:
            mensaje (str): Mensaje original en texto plano
            mensaje_cifrado (str): Mensaje ya cifrado
            
        Returns:
            str: mensaje_cifrado|||HASH|||hash_sha256
        """
        try:
    ...
    def validar_y_extraer(mensaje_con_hash):
        """
        Valida el hash y extrae el mensaje cifrado
        
        Args:
            mensaje_con_hash (str): mensaje_cifrado|||HASH|||hash_sha256
            
        Returns:
            tuple: (mensaje_cifrado, hash_valido)
                   mensaje_cifrado (str): El mensaje cifrado sin el hash
                   hash_valido (bool): True si el formato es correcto y el hash esta presente
        """
        try:
    ...
    def validar_integridad(mensaje_descifrado, hash_recibido):
        """
        Valida que el hash del mensaje descifrado coincida con el hash recibido
        
        Args:
            mensaje_descifrado (str): Mensaje ya descifrado
            hash_recibido (str): Hash SHA-256 recibido
